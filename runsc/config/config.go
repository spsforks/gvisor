...
type Config struct {
	...
	// NetworkThreadsPerChannel controls the number of threads used to handle
	// packets on a single network channel. A higher number can help handle many
	// simultaneous connections. If this is 0, runsc will divide GOMAXPROCS evenly
	// among each network channel.
	NetworkThreadsPerChannel int `flag:"network-threads-per-channel"`
